...
const locationId =
  "ef8801feddd0b4136ac70c3b133451e22754037d3dbef11a91234d3442a9b7e6";
...
axios
  ...
  .then((response) => {
    ...
    const statusCode = htmlData("#regionHeader")
      .attr("class")
      .split(" ")[2]
      .split("-")[2];
    const icon = weatherIcons[statusCode]
      ? weatherIcons[statusCode]
      : weatherIcons.default;
    const tempFeel = htmlData(
      "div[data-testid='FeelsLikeSection'] > span > span[data-testid='TemperatureValue']",
    ).text();
    const tempFeelText = `Как будто ${tempFeel}c`;
    const tempMin = htmlData(
      "div[data-testid='wxData'] > span[data-testid='TemperatureValue']",
    )
      .eq(0)
      .text();
    const tempMax = htmlData(
      "div[data-testid='wxData'] > span[data-testid='TemperatureValue']",
    )
      .eq(1)
      .text();
    const tempMinMax = `  ${tempMin}\t\t  ${tempMax}`;
    const windSpeed = htmlData("span[data-testid='Wind']")
      .text()
      .split("\n")[1];
    ...
    const airQualityIndex = htmlData(
      "text[data-testid='DonutChartValue']",
    ).text();
